...
activity_organizations = Table("activity_organizations", Base.metadata,
...
activity_contacts = Table("activity_contacts", Base.metadata,
    Column("activity_id", String(36), ForeignKey("activities.id", ondelete="CASCADE"), primary_key=True),
    Column("contact_id", String(36), ForeignKey("contacts.id", ondelete="CASCADE"), primary_key=True))

activity_opportunities = Table("activity_opportunities", Base.metadata,
    Column("activity_id", String(36), ForeignKey("activities.id", ondelete="CASCADE"), primary_key=True),
...
class Activity(Base, TenantMixin, AuditMixin, SoftDeleteMixin):
    ...
    organizations = relationship("Organization", secondary=activity_organizations, back_populates="linked_activities", lazy="selectin")
    contact_id = Column(String(36), ForeignKey("contacts.id"), nullable=True, index=True)
    contacts = relationship("Contact", secondary=activity_contacts, back_populates="linked_activities", lazy="selectin")
    opportunity_id = Column(String(36), ForeignKey("opportunities.id"), nullable=True, index=True)
    opportunities = relationship("Opportunity", secondary=activity_opportunities, back_populates="linked_activities", lazy="selectin")
