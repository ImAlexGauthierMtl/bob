...
class OpportunityRepository:
    ...
    def create(self, opp: Opportunity) -> Opportunity:
        self.db.add(opp); self.db.commit(); self.db.refresh(opp); return opp
    def get_by_id(self, opp_id: str, tenant_id: str) -> Optional[Opportunity]:
        return self.db.query(Opportunity).filter(Opportunity.id == opp_id, Opportunity.tenant_id == tenant_id, Opportunity.is_deleted == False).first()
    def list_all(self, tenant_id: str, skip: int = 0, limit: int = 50, organization_id: Optional[str] = None, stage: Optional[str] = None) -> List[Opportunity]:
        ...
        if organization_id: q = q.filter(Opportunity.organization_id == organization_id)
    ...
    def update(self, opp: Opportunity) -> Opportunity:
        opp.version += 1; self.db.commit(); self.db.refresh(opp); return opp
    def soft_delete(self, opp: Opportunity, deleted_by: str, reason: Optional[str] = None) -> Opportunity:
        from datetime import datetime, timezone
        opp.is_deleted = True; opp.deleted_at = datetime.now(timezone.utc); opp.deleted_by = deleted_by; opp.deleted_reason = reason; opp.version += 1
        self.db.commit(); self.db.refresh(opp); return opp
    def add_product(self, line_item):
        self.db.add(line_item); self.db.commit(); self.db.refresh(line_item); return line_item
    def list_products(self, opp_id: str, tenant_id: str) -> list:
        ...
        return self.db.query(OpportunityProduct).filter(OpportunityProduct.opportunity_id == opp_id, OpportunityProduct.tenant_id == tenant_id).all()
    def get_product_line(self, line_id: str, tenant_id: str):
        from app.domain.entities.opportunity_product import OpportunityProduct
        return self.db.query(OpportunityProduct).filter(OpportunityProduct.id == line_id, OpportunityProduct.tenant_id == tenant_id).first()
    def remove_product(self, line_item) -> None:
        self.db.delete(line_item); self.db.commit()
