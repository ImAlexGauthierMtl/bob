...
from app.presentation.schemas.crm_schemas import DepartmentCreate, DepartmentUpdate, DepartmentResponse
...
async def create_department(data: DepartmentCreate, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    repo = DepartmentRepository(db)
...
async def get_department(dept_id: str, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    repo = DepartmentRepository(db)
...
async def update_department(dept_id: str, data: DepartmentUpdate, user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    repo = DepartmentRepository(db)
    dept = repo.get_by_id(dept_id, user["tenant_id"])
    if not dept: raise HTTPException(status_code=404, detail="Department not found")
    for k, v in data.model_dump(exclude_unset=True).items(): setattr(dept, k, v)
