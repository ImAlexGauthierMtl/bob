"""Product Backend API — CRUD for products. Port: 9006."""
...
async def lifespan(app: FastAPI):
    from app.infrastructure.persistence.models import product  # noqa: F401
    from app.infrastructure.database import init as db_init
    db_init("product-backend")
    if hasattr(event_bus, 'start_listening'):
        await event_bus.start_listening()
    logger.info("product_backend_started", port=9006)
    yield
    if hasattr(event_bus, 'close'):
        await event_bus.close()


app = FastAPI(title="Product Backend API", version="1.0.0", lifespan=lifespan)
