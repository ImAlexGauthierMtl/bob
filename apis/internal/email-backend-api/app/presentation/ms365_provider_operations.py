"""MS365 provider operations used by the provider use case adapter."""
import asyncio
import json
import os
...
class MS365ProviderOperations:

    async def get_auth_url(self, current_user: dict = None):
    ...
    async def get_connection(self, request: Request, current_user: dict = None):
        conn = await connection_client.get_by_user(current_user["user_id"], forward_headers=request.headers)
        if not conn:
            return None
        return conn

    async def disconnect(self, request: Request, current_user: dict = None):
        conn = await connection_client.get_by_user(current_user["user_id"], forward_headers=request.headers)
        if not conn:
    ...
    async def list_emails(self, request: Request, skip: int = Query(0, ge=0), limit: int = Query(50, ge=1, le=100), folder: Optional[str] = None, search: Optional[str] = None, smart_label: Optional[str] = None, linked_contact_id: Optional[str] = None, current_user: dict = None):
    ...
    async def get_email(self, email_id: str, request: Request, current_user: dict = None):
    ...
    async def reply_email(self, email_id: str, reply_request: ReplyEmailRequest, request: Request, current_user: dict = None):
        conn = await connection_client.get_by_user(current_user["user_id"], forward_headers=request.headers)
        if not conn or not conn.get("is_active"):
            raise HTTPException(status_code=400, detail="No active MS365 connection.")
    ...
    async def forward_email(self, email_id: str, fwd_request: ForwardEmailRequest, request: Request, current_user: dict = None):
        conn = await connection_client.get_by_user(current_user["user_id"], forward_headers=request.headers)
        if not conn or not conn.get("is_active"):
            raise HTTPException(status_code=400, detail="No active MS365 connection.")
    ...
    async def list_events(self, request: Request, skip: int = Query(0, ge=0), limit: int = Query(50, ge=1, le=100), from_date: Optional[datetime] = None, to_date: Optional[datetime] = None, current_user: dict = None):
    ...
    async def get_event(self, event_id: str, request: Request, current_user: dict = None):
