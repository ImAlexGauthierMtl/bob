...
from app.presentation.deps import get_pipedream_provider_use_cases
...
async def create_pipedream_token(
    ...
    try:
        token_payload = await use_cases.create_token(
            current_user,
            integration_key,
            request.headers,
...
async def get_connect_url(
    ...
    try:
        return PipedreamConnectUrlResponse.model_validate(
            await use_cases.connect_url(current_user, integration_key, redirect_uri, request.headers)
        )
    except httpx.HTTPStatusError as exc:
...
async def list_connections(
    ...
    try:
        return PipedreamConnectionListResponse(
            items=[
                PipedreamConnectionResponse.model_validate(account)
                for account in await use_cases.list_connections(current_user, integration_key, request.headers)
            ]
        )
    except httpx.HTTPStatusError as exc:
...
async def delete_connection(
    ...
    try:
        removed = await use_cases.delete_connection(connection_id)
        if not removed:
...
async def run_action(
    ...
    try:
        result = await use_cases.run_action(
            current_user,
            body.action_key or action_key,
            str(integration_key),
            body.input,
            request.headers,
...
async def proxy_send_email(
    ...
    return await run_action(action_id, PipedreamActionRunRequest(input=body), request, current_user, use_cases)
...
async def trigger_pipedream_sync(
    ...
    return await run_action(action_id, PipedreamActionRunRequest(input={}), Request({"type": "http", "headers": []}), current_user, use_cases)


@router.get("/config", response_model=PipedreamConfigResponse)
async def get_pipedream_config(
    current_user: dict = Depends(get_current_user),
    use_cases: PipedreamProviderUseCases = Depends(get_pipedream_provider_use_cases),
):
    return PipedreamConfigResponse.model_validate(use_cases.get_config())
...
async def update_pipedream_config(
    ...
    return PipedreamConfigResponse.model_validate(use_cases.update_config(request.model_dump()))
