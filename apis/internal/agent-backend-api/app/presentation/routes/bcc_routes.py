...
async def add_skill(
    role_id: str, data: BccSkillCreate,
    current_user: dict = Depends(get_current_user),
    db: Session = Depends(get_db),
):
    role = db.query(BccRole).filter(BccRole.id == role_id, BccRole.tenant_id == current_user["tenant_id"]).first()
    if not role:
        raise HTTPException(status_code=404, detail="Role not found")
...
async def get_skill(skill_id: str, current_user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    skill = db.query(BccSkill).filter(BccSkill.id == skill_id, BccSkill.tenant_id == current_user["tenant_id"]).first()
    if not skill:
...
async def update_skill(skill_id: str, data: BccSkillUpdate, current_user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    skill = db.query(BccSkill).filter(BccSkill.id == skill_id, BccSkill.tenant_id == current_user["tenant_id"]).first()
    if not skill:
        raise HTTPException(status_code=404, detail="Skill not found")
    for key, value in data.model_dump(exclude_none=True).items():
...
async def delete_skill(skill_id: str, current_user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    skill = db.query(BccSkill).filter(BccSkill.id == skill_id, BccSkill.tenant_id == current_user["tenant_id"]).first()
    if not skill:
        raise HTTPException(status_code=404, detail="Skill not found")
    db.delete(skill)
...
async def add_task(
    role_id: str, data: BccTaskCreate,
    current_user: dict = Depends(get_current_user),
    db: Session = Depends(get_db),
):
    role = db.query(BccRole).filter(BccRole.id == role_id, BccRole.tenant_id == current_user["tenant_id"]).first()
...
async def get_task(task_id: str, current_user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    task = db.query(BccTask).filter(BccTask.id == task_id, BccTask.tenant_id == current_user["tenant_id"]).first()
    if not task:
...
async def update_task(task_id: str, data: BccTaskUpdate, current_user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    task = db.query(BccTask).filter(BccTask.id == task_id, BccTask.tenant_id == current_user["tenant_id"]).first()
    if not task:
        raise HTTPException(status_code=404, detail="Task not found")
    for key, value in data.model_dump(exclude_none=True).items():
...
async def add_task_step(task_id: str, data: BccTaskStepCreate, current_user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    task = db.query(BccTask).filter(BccTask.id == task_id, BccTask.tenant_id == current_user["tenant_id"]).first()
    if not task:
        raise HTTPException(status_code=404, detail="Task not found")
...
async def add_resource(skill_id: str, data: BccResourceCreate, current_user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    skill = db.query(BccSkill).filter(BccSkill.id == skill_id, BccSkill.tenant_id == current_user["tenant_id"]).first()
    if not skill:
        raise HTTPException(status_code=404, detail="Skill not found")
...
async def add_milestone(role_id: str, data: BccMilestoneCreate, current_user: dict = Depends(get_current_user), db: Session = Depends(get_db)):
    role = db.query(BccRole).filter(BccRole.id == role_id, BccRole.tenant_id == current_user["tenant_id"]).first()
    if not role:
        raise HTTPException(status_code=404, detail="Role not found")
...
async def get_profile_section(
    entity_type: str, entity_id: str, section: str,
    current_user: dict = Depends(get_current_user),
    db: Session = Depends(get_db),
):
    _validate_entity_type(entity_type)
