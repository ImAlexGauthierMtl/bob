"""Agent Runtime Backend API. Port: 9013."""
...
settings = get_settings("agent-runtime-backend")
...
def create_internal_session_signer() -> InternalSessionContextSigner:
    ...
    if not secret:
        raise RuntimeError("INTERNAL_SESSION_SECRET is required for agent-runtime-backend" + "-api")
...
async def lifespan(app: FastAPI):
    ...
    from app.infrastructure.persistence.models import agent_runtime  # noqa: F401

    db_init("agent-runtime-backend")
    if hasattr(event_bus, "start_listening"):
        await event_bus.start_listening()
    logger.info("agent_runtime_backend_started", port=9013)
    yield
    if hasattr(event_bus, "close"):
        await event_bus.close()


app = FastAPI(title="Agent Runtime Backend API", version="0.1.0", lifespan=lifespan)
setup_cors(app, "agent-runtime-backend")
app.add_middleware(RequestLoggingMiddleware)
app.middleware("http")(InternalSessionContextMiddleware(create_internal_session_signer()))
app.include_router(monitoring_router, tags=["monitoring"])

from app.presentation.routes.agent_runtime_routes import router as agent_runtime_router

app.include_router(agent_runtime_router, tags=["agent-runtime"])
