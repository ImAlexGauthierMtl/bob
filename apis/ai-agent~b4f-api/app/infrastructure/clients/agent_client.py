...
class BccClient:
    ...
    def __init__(self):
        self._client = create_service_client("agent~backend-api")
...
class BobSettingsClient:
    """HTTP client for Bob Settings on agent~backend-api."""

    def __init__(self):
        self._client = create_service_client("agent~backend-api")

    async def get(self, forward_headers=None):
        resp = await self._client.get("/api/v1/bob/settings", forward_headers=forward_headers)
        resp.raise_for_status()
        return resp.json()

    async def update(self, data, forward_headers=None):
        resp = await self._client.put("/api/v1/bob/settings", json=data, forward_headers=forward_headers)
        resp.raise_for_status()
        return resp.json()

    async def reset(self, forward_headers=None):
        resp = await self._client.delete("/api/v1/bob/settings", forward_headers=forward_headers)
        return resp.status_code == 204
...
class CapabilityClient:
    """HTTP client for Capabilities on agent~backend-api."""

    def __init__(self):
        self._client = create_service_client("agent~backend-api")

    async def list_catalog(self, forward_headers=None):
        resp = await self._client.get("/api/v1/capabilities/catalog", forward_headers=forward_headers)
        resp.raise_for_status()
        return resp.json()

    async def get_user_capabilities(self, user_id, forward_headers=None):
        resp = await self._client.get(f"/api/v1/capabilities/users/{user_id}", forward_headers=forward_headers)
        resp.raise_for_status()
        return resp.json()

    async def get_my_capabilities(self, forward_headers=None):
        resp = await self._client.get("/api/v1/capabilities/me", forward_headers=forward_headers)
        resp.raise_for_status()
        return resp.json()

    async def assign(self, user_id, data, forward_headers=None):
        resp = await self._client.post(f"/api/v1/capabilities/users/{user_id}/assign", json=data, forward_headers=forward_headers)
        resp.raise_for_status()
        return resp.json()

    async def check(self, capability_code, forward_headers=None):
        resp = await self._client.post(f"/api/v1/capabilities/check?capability_code={capability_code}", forward_headers=forward_headers)
        resp.raise_for_status()
        return resp.json()
...
bob_settings_client = BobSettingsClient()
