...
from app.application.use_cases.user_use_cases import (
    UpdateProfileUseCase, ListUsersUseCase, CreateUserByAdminUseCase,
)
from app.presentation.schemas.user_schemas import (
    UserUpdateRequest, UserCreateByAdminRequest, UserResponse, UserListResponse,
)
...
async def update_my_profile(
    ...
    try:
        user = use_case.execute(user_id=current_user["user_id"], **data.model_dump(exclude_unset=True))
...
async def list_users(
    ...
    users, total = use_case.execute(tenant_id=current_user["tenant_id"], skip=skip, limit=limit)
...
async def create_user(
    ...
    try:
        user = use_case.execute(
            ...
            tenant_id=current_user["tenant_id"], role=data.role,
