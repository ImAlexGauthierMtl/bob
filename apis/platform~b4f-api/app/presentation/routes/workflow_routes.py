...
# UserRepository removed - cross-service dependency
...
async def run_workflow(
    ...
    """Execute a workflow."""
    repo = WorkflowRepository(db)
    # Cross-service: query user via raw SQL
    ...
    result = db.execute(text("SELECT id, tenant_id FROM users WHERE email = :email"), {"email": current_user["email"]}).first()
