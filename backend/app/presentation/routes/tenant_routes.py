...
async def list_tenants(
    ...
    """List all tenants with optional search and status filter."""
...
async def provision_tenant(
    ...
    """Provision a tenant — create its first admin user with full RBAC."""
    repo = TenantRepository(db)
    tenant = repo.get_by_id(tenant_id)
    if not tenant:
        raise HTTPException(status_code=404, detail="Tenant not found")
    ...
    # 1. Seed roles & permissions for this tenant (idempotent)
    ...
    # 3. Assign admin role to the new user
