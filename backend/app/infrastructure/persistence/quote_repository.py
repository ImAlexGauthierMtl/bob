...
class QuoteRepository:
    ...
    def list_all(self, tenant_id: str, skip: int = 0, limit: int = 50, opportunity_id: Optional[str] = None, organization_id: Optional[str] = None) -> List[Quote]:
        query = self.db.query(Quote).filter(Quote.tenant_id == tenant_id, Quote.is_deleted == False)
        if opportunity_id:
            query = query.filter(Quote.opportunity_id == opportunity_id)
        if organization_id:
            query = query.filter(Quote.organization_id == organization_id)
