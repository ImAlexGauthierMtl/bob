...
class MS365Connection(Base, TenantMixin, AuditMixin, SoftDeleteMixin):
    ...
    # Relationships
    synced_emails = relationship("SyncedEmail", back_populates="connection", passive_deletes=True)
    synced_events = relationship("SyncedEvent", back_populates="connection", passive_deletes=True)
