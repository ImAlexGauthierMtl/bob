...
- `product-backend-api`, `contact-backend-api`, `org-backend-api`, `opportunity-backend-api`, `user-backend-api`, `usage-backend-api`, `kb-backend-api`, `workflow-backend-api`, les routes Agent `capability`/`client_map`/`training`/`bob_settings` et les routes Email core suivent le même pattern de routes HTTP minces avec use cases applicatifs.
...
3. Répliquer le pattern validé sur les routes restantes: `email-backend-api` provider/Membrane, `agent-backend-api/bcc_routes.py` et `auth-b4f-api`.
