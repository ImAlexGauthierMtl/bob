...
3. Répliquer le pattern validé sur `activity-backend-api`, `product-backend-api` et `contact-backend-api` vers `org-backend-api`, `opportunity-backend-api` et les routes longues `email-backend-api`.
