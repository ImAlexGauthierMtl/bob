...
- `product-backend-api`, `contact-backend-api` et `org-backend-api` suivent le même pattern de routes HTTP minces avec use cases applicatifs.
...
3. Répliquer le pattern validé sur `activity-backend-api`, `product-backend-api`, `contact-backend-api` et `org-backend-api` vers `opportunity-backend-api` et les routes longues `email-backend-api`.
