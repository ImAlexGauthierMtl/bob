...
3. Répliquer le pattern validé sur `activity-backend-api` et `product-backend-api` vers `contact-backend-api`, `org-backend-api`, `opportunity-backend-api` et les routes longues `email-backend-api`.
