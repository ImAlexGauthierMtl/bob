...
- `product-backend-api`, `contact-backend-api`, `org-backend-api`, `opportunity-backend-api`, `user-backend-api` et `usage-backend-api` suivent le même pattern de routes HTTP minces avec use cases applicatifs.
...
3. Répliquer le pattern validé sur `activity-backend-api`, `product-backend-api`, `contact-backend-api`, `org-backend-api`, `opportunity-backend-api`, `user-backend-api` et `usage-backend-api` vers les routes longues `email-backend-api`, `agent-backend-api` et `auth-b4f-api`.
