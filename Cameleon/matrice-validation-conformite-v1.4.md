...
| A-08 | Services externes seulement côté Backend | § 2.2-2.3 | `cde-check-api-boundaries` | Recherche clients MS365/Pipedream dans B4F | OK | Pipedream est le provider actif: B4F `/pipedream/*` proxy mince vers `email-backend-api` `/api/v1/provider/pipedream`; ancien provider GetMembrane retiré du runtime et du code provider; scan GetMembrane actif sans résultat; scan B4F sans client externe provider (`PipedreamClient`, `create_connect_token`, `graph.microsoft`, `MS365GraphService` absents hors tests); clients externes sous `apis/internal/email-backend-api/app/infrastructure/external`; tests locaux ciblés OK: `communication-b4f-api` 7 passed, `email-backend-api` 11 passed, `npm run build` OK, captures UI `captures/pipedream-integrations-page.png` et `captures/pipedream-config-modal.png` |
...
| CA-03 | `application/` sans framework | § 12.3 | `cde-check-clean-architecture` | Recherche imports FastAPI/SQLAlchemy/Pydantic dans application | OK | Import-linter garde les contrats application/domain sans framework; les nouveaux `application/use_cases` de `activity`, `product`, `contact`, `org`, `opportunity`, `user`, `usage`, `kb`, `workflow`, `agent` et `email` n'importent pas FastAPI/SQLAlchemy/Pydantic. Des services legacy `email-backend-api/app/application/services/*` importent encore `app.infrastructure` et restent à porter derrière des ports lors de la passe provider. |
| CA-04 | Routes sans logique métier | § 12.3 | `cde-check-clean-architecture` | Lire routes longues/complexes | VIOLATION | `activity-backend-api`, `product-backend-api`, `contact-backend-api`, `org-backend-api`, `opportunity-backend-api`, `user-backend-api`, `usage-backend-api`, `kb-backend-api`, `workflow-backend-api`, les routes Agent `capability`, `client_map`, `training`, `bob_settings`, les routes Email core et `email-backend-api` `membrane_routes.py` ont maintenant des routes minces qui appellent des use cases; la violation reste ouverte car plusieurs routes contiennent encore persistence/logique métier, exemples: `provider_pipedream_routes.py`, `provider_ms365_routes.py`, `bcc_routes.py`, `auth_routes.py` |
