...
| F-03 | Aucun HTTP hors Effects | § 3.1-3.6 | `cde-check-frontend-ngrx` | Recherche `HttpClient`/services dans components | VIOLATION | Le dashboard est migre vers `loadCrmDashboard` -> `CrmEffects` -> `CrmB4fService` et ne contient plus de `.subscribe()`; `GET http://localhost:8002/dashboard/summary` retourne `HTTP 200`; `rg "HttpClient|\\.subscribe\\(" frontend/src/app -g '*.ts'` montre encore des composants/pages legacy hors Effects |
| F-04 | Templates utilisent `| async` | § 3.1 | `cde-check-frontend-ngrx` | Lire templates critiques | VIOLATION | `frontend/src/app/pages/dashboard/dashboard.html` lit `vm$ | async`; capture locale `captures/cde-dashboard-ngrx.png` sans banniere `Request failed`; migration legacy incomplete ailleurs, voir `Cameleon/audit-frontend-ngrx-v1.4.md` |
