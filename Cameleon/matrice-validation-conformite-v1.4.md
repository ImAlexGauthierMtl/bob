...
| M-10 | § 12 | Clean Architecture API | `cde-check-clean-architecture` | `domain/application/infrastructure/presentation`, import-linter, tests par couche | OK | à corriger |
...
| CA-04 | Routes sans logique métier | § 12.3 | `cde-check-clean-architecture` | Lire routes longues/complexes | OK | Les routes épaisses ciblées sont converties: `provider_pipedream_routes.py`, `provider_ms365_routes.py`, `agent-backend-api/app/presentation/routes/bcc_routes.py` et `auth-b4f-api/app/presentation/routes/auth_routes.py` délèguent aux use cases; validations locales OK: `email-backend-api` 11 passed + import-linter, `agent-backend-api` 12 passed + import-linter, `auth-b4f-api` 8 passed + import-linter |
