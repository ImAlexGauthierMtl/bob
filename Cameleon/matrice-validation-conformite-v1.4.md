...
| CA-04 | Routes sans logique métier | § 12.3 | `cde-check-clean-architecture` | Lire routes longues/complexes | VIOLATION | `activity-backend-api`, `product-backend-api`, `contact-backend-api`, `org-backend-api`, `opportunity-backend-api`, `user-backend-api`, `usage-backend-api`, `kb-backend-api`, `workflow-backend-api`, les routes Agent `capability`, `client_map`, `training`, `bob_settings`, et les routes Email core `connection`, `email`, `event`, `email_contact`, `integration_settings`, `smart_label` ont maintenant des routes minces qui appellent des use cases; la violation reste ouverte car plusieurs routes contiennent encore persistence/logique métier, exemples: `provider_membrane_routes.py`, `provider_ms365_routes.py`, `membrane_routes.py`, `bcc_routes.py`, `auth_routes.py` |
