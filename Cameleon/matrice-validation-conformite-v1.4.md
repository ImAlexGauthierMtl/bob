...
| CI-06 | Couverture 85 %, JUnit, Cobertura | § 4.9 | `dx_base_check_cicd_coverage_85_percent` | Lire jobs test | VIOLATION | `activity-backend-api`, `ai-agent-b4f-api`, `auth-b4f-api`, `communication-b4f-api`, `contact-backend-api`, `crm-b4f-api`, `kb-b4f-api`, `opportunity-backend-api`, `org-backend-api`, `platform-b4f-api`, `product-backend-api`, `usage-backend-api`, `user-backend-api` et `workflow-backend-api` packagés avec tests et seuil 85 % validés localement + commande template; les autres APIs internes CDE restent à packager et sont encore skippées |
