...
| A-06 | B4F porte logique métier, pas proxy CRUD 1:1 | § 2.2, § 2.8 | `cde-check-api-boundaries` | Lire routes B4F et clients backend | VIOLATION | `crm-b4f-api` ajoute `/dashboard/summary` (`crm-b4f-api` 9 passed) et `kb-b4f-api` ajoute `/kb/home` (`kb-b4f-api` 9 passed); l'audit `Cameleon/audit-api-boundaries-v1.4.md` garde l'ecart ouvert pour une partie de `ai-agent-b4f-api`, `communication-b4f-api` et `platform-b4f-api` |
