...
| A-06 | B4F porte logique métier, pas proxy CRUD 1:1 | § 2.2, § 2.8 | `cde-check-api-boundaries` | Lire routes B4F et clients backend | VIOLATION | `crm-b4f-api` ajoute `/dashboard/summary` (9 passed), `kb-b4f-api` ajoute `/kb/home` (9 passed), `communication-b4f-api` ajoute `/integrations/overview` (7 passed); l'audit garde l'ecart ouvert pour une partie de `ai-agent-b4f-api` et `platform-b4f-api` |
