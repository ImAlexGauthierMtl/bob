...
| A-06 | B4F porte logique métier, pas proxy CRUD 1:1 | § 2.2, § 2.8 | `cde-check-api-boundaries` | Lire routes B4F et clients backend | OK | Audit `Cameleon/audit-api-boundaries-v1.4.md`: `auth-b4f-api` porte JWT/session/rate-limit, `crm-b4f-api` `/dashboard/summary` (9 passed), `kb-b4f-api` `/kb/home` (9 passed), `communication-b4f-api` `/integrations/overview` (7 passed), `ai-agent-b4f-api` chat/analyse comportementale, `platform-b4f-api` `/overview` et override workflow (9 passed) |
