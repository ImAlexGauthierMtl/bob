...
| A-06 | B4F porte logique métier, pas proxy CRUD 1:1 | § 2.2, § 2.8 | `cde-check-api-boundaries` | Lire routes B4F et clients backend | VIOLATION | `crm-b4f-api` ajoute `/dashboard/summary` comme composition multi-backend testée (`crm-b4f-api` 9 passed); l'audit `Cameleon/audit-api-boundaries-v1.4.md` garde l'ecart ouvert pour `kb-b4f-api`, une partie de `ai-agent-b4f-api`, `communication-b4f-api` et `platform-b4f-api` |
